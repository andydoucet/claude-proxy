...
# subscriptions (Claude Max, OpenAI Max / Codex, Antigravity)
...
#   Non-interactive: bash setup-linux.sh --non-interactive --providers claude,codex
...
get_models_for_provider() {
    case "$1" in
        claude)
            ...
            # Fable 5 is available as a first-class direct model but intentionally
            # does not masquerade as Opus.
        ...
        if [ -n "$CLI_PROVIDERS" ]; then
            ...
            for p in "${SELECTED_PROVIDERS[@]}"; do
                ...
                if ! $valid; then
                    ...
                    error "Valid: claude, codex, antigravity"
                    exit 1
                fi
            done
        else
